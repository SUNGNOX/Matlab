...
%2017-8-18
function higher=plot_zlq_function2(path,file,FitMethod,point_num)
    Landscape=0;%横向差分
    Portrait=1;%纵向差分
...
%     二次曲面拟合
    ...
    D0=Y(sc(1):sc(end), sr(1):sr(end))-YFIT(sc(1):sc(end), sr(1):sr(end));
    [b0,~]=find(D0>0);
    [s0,~]=find(D0<0);
    if length(b0)>length(s0)
        for w=1:point_num
            h(w)=max(max(D0));
            [a, b]=find(D0==max(max(D0)));
            D0(a, b)=0;
            inner_point(w,2) = a+sc(1)-1;
            inner_point(w,1) = b+sr(1)-1;
        end
    else
        for w=1:point_num
            D0=-D0;
